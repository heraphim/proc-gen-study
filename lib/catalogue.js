...
export function openCatalogue(root) {
  ...
  const sourceAnn = annotation('sources');
  const sourceMeta = {
    about: sourceAnn._about ?? null,
    rule: sourceAnn._rule ?? null,
    relations: sourceAnn._relations ?? {},
  };
  ...
  function entries() {
    const rows = all(`
      SELECT e.id, e.name, e.description, e.tier, e.output_type, e.input_class,
             e.compute_cost, e.addressing, e.runs_at, e.realtime, e.difficulty, e.confidence,
  ...
  /* The axis layer: the prose from the annotation file joined to counts from the column it
     names. Counts are computed here rather than in the browser because an axis with a value
     nothing takes is a finding, and it should be visible without the reader summing anything. */
  function axisLayer() {
    const ann = annotation('axes');
    return {
      about: ann._about ?? null,
      test: ann._test ?? null,
      whyNotConcepts: ann._why_not_concepts ?? null,
      rules: ann._rules ?? {},
      stillEmpty: ann._still_empty ?? {},
      axes: (ann.axes ?? []).map(a => {
        const counts = Object.fromEntries(
          all(`SELECT ${a.column} AS v, COUNT(*) AS n FROM entry GROUP BY ${a.column}`)
            .map(r => [r.v, r.n]));
        return {
          id: a.id, column: a.column, name: a.name, question: a.question,
          eli5: a.eli5, why: a.why,
          rule: { about: a.rule?._about ?? null, default: a.rule?.default ?? null,
                  precedence: a.rule?.precedence ?? [], byConcept: a.rule?.by_concept ?? {} },
          overrides: Object.values(a.overrides ?? {}).reduce((n, o) => n + Object.keys(o).length, 0),
          values: (a.values ?? []).map(v => ({ ...v, notable: !!v.notable, count: counts[v.id] ?? 0 })),
        };
      }),
    };
  }
  ...
  function bootstrap() {
    ...
    return {
      ...
      // Undated last, not first. The year column is null where a technique has no honest date
      // (shader folklore, mostly), and SQLite sorts nulls to the front, which put two undated
      // rows ahead of 1949 and broke the chronology each concept group otherwise reads as.
      algorithms: all(`
        SELECT id, name, concept_tag, year, authors, summary, description, eli5, tier, source_type, citation, url
        FROM algorithm ORDER BY year IS NULL, year, position`)
...
export const ROUTES = [
  ...
  '/definitions', '/case-studies', '/pitfalls', '/tools', '/sources', '/sql',
];
