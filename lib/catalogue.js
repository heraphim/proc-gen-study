...
export function openCatalogue(root) {
  ...
  const conceptMeta = {
    ...
    additions: (conceptAnn.additions ?? []).map(a => ({
      id: a.id, why: a.why, importable: a.importable, absence: a.absence ?? null,
    })),
    corrections: conceptAnn.corrections ?? [],
  };
  ...
  function bootstrap() {
    ...
    return {
      ...
      corrections: all(`
        ...
        FROM correction ORDER BY position`),
      implementations: all(`
